...
async function testQueueSync() {
  ...
  try {
    ...
    const wsResponse = await fetch('http://localhost:3000/debug/websocket-state');
    ...
    if (wsData.currentSession?.queue) {
      ...
      wsData.currentSession.queue.forEach((item, i) => {
        console.log(`     ${i+1}. "${item.title}" by ${item.artist} (${item.status}) - added by ${item.addedBy}`);
      });
    }
    ...
    if (apiData.success) {
      ...
      console.log('   Current song:', apiData.data.currentSong?.mediaItem?.title || 'None');
      
      if (apiData.data.queue?.length > 0) {
        console.log('   Queue items:');
        apiData.data.queue.forEach((item, i) => {
          console.log(`     ${i+1}. "${item.mediaItem.title}" by ${item.mediaItem.artist} (${item.status}) - added by ${item.addedBy}`);
        });
      }
    } else {
    ...
    const apiQueueLength = apiData.success ? (apiData.data.queue?.length || 0) : 0;
    
    if (wsQueueLength > 0 && apiQueueLength === 0) {
      console.log('   ❌ ISSUE CONFIRMED: WebSocket has queue items but API session manager is empty');
