...
export function LyricsIndicator({ 
  ...
  const classes = `${baseClasses} ${sizeClasses[size][variant]} ${variantClasses[variant]} ${className}`;

  if (variant === "badge") {
    return (
      <span className={classes} title={hasLyrics ? "Lyrics available" : "No lyrics available"}>
        {hasLyrics ? (
          <>
